...
async function load(handle: FileHandle, generation = operations.begin()) {
  ...
  const data = await readFileData(hiraya, handle); if (!operations.isLatest(generation)) return; const nextUrl = URL.createObjectURL(new Blob([data], { type: mimeType })); let element: HTMLElement;
