...
if (import.meta.main) {
  ...
  if (!server || !storeRoot || kind !== "store" && kind !== "system" || !slug || !archive || !archive.endsWith(".hiraya.app")) usage();
  try { await releaseApp(server, storeRoot, kind, slug, archive); } catch (error) {
