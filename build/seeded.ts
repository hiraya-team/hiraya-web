...
export function seededDesktopPlugin(projectRoot: string, configuredDirectory?: string): Plugin {
  ...
  return {
    name: "hiraya-seeded-desktop",
    enforce: "pre",
    async configResolved() {
      if (!configuredDirectory?.trim()) return;
      const root = await realpath(projectRoot);
      const candidate = path.resolve(root, configuredDirectory);
      if (!isInside(root, candidate)) throw new Error("HIRAYA_SEEDED_DIR must point to a directory inside the repository.");
      const sourceStats = await lstat(candidate);
      if (!sourceStats.isDirectory() || sourceStats.isSymbolicLink()) throw new Error("HIRAYA_SEEDED_DIR must point to a regular directory, not a symbolic link.");
      const resolvedSource = await realpath(candidate);
      if (!isInside(root, resolvedSource)) throw new Error("HIRAYA_SEEDED_DIR resolves outside the repository.");
    ...
    async load(id) {
      ...
      if (!manifestStats.isFile() || manifestStats.isSymbolicLink()) {
        throw new Error(`Seeded desktop manifest at ${manifestPath} must be a regular file, not a symbolic link.`);
      }
      ...
      for (const [index, entry] of manifest.entries.entries()) {
        ...
        if (unresolvedStats.isSymbolicLink()) throw new Error(`Seeded file “${entry.name}” cannot be a symbolic link.`);
        const resolvedAsset = await realpath(assetPath);
        if (!isInside(sourceDirectory, resolvedAsset)) throw new Error(`Seeded file “${entry.name}” resolves outside HIRAYA_SEEDED_DIR.`);
        const stats = await lstat(resolvedAsset);
        if (!stats.isFile()) throw new Error(`Seeded file “${entry.name}” does not refer to a regular file.`);
        if (stats.size !== entry.size) throw new Error(`Seeded file “${entry.name}” has size ${stats.size}, but its manifest declares ${entry.size}.`);
