...
- **Keep portable task lists in Todo.** Install the experimental **Todo** app to add, prioritize, date, filter, complete, and safely save `.hiraya.todo` task lists. Todo, the other experimental apps, **Text Editor**, and **Markdown Preview** now share familiar Hiraya controls while keeping their specialized workflows.
