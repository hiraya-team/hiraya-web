...
An **area** is a region derived from item and window coordinates on one continuous desktop. Icon areas use the largest whole 24-pixel sub-grid extent that fits the viewport, leaving at most 23 pixels of non-placement space on the right and bottom; windows continue to use the full viewport. Areas are not named containers, folders, or separately saved records. Moving the last contents out of an area can make that derived area disappear.
