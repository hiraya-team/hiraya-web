...
export function injectSandboxUiRuntime(document: Document, uiRuntime: SandboxUiRuntime, csp: string, navigationToken: string): void {
  ...
  navigationGuard.src = dataURL(new TextEncoder().encode(`(()=>{const token=${JSON.stringify(navigationToken)};const notify=phase=>parent.postMessage({type:"hiraya:sandbox-navigation",token,phase},"*");const activate=()=>parent.postMessage({type:"hiraya:sandbox-activate",token},"*");const pointer=event=>parent.postMessage({type:"hiraya:sandbox-pointer",token,observation:{phase:event.type,x:event.clientX,y:event.clientY,button:event.button,buttons:event.buttons,altKey:event.altKey,ctrlKey:event.ctrlKey,metaKey:event.metaKey,shiftKey:event.shiftKey,pointerId:event.pointerId||0,pointerType:event.pointerType||"mouse"}},"*");for(const type of ["pointerdown","pointermove","pointerup","pointercancel","contextmenu"])addEventListener(type,pointer,{capture:true});addEventListener("message",event=>{const message=event.data;if(message?.type==="hiraya:wallpaper-pointer"&&message.token===token)dispatchEvent(new CustomEvent("hiraya:wallpaper-pointer",{detail:message.observation}))});addEventListener("pointerdown",activate,{capture:true});addEventListener("focus",activate);addEventListener("load",()=>notify("load"),{once:true});addEventListener("beforeunload",()=>{stop();notify("navigation")},{capture:true,once:true})})()`), "text/javascript");
