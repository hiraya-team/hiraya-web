...
describe("apps contracts", () => {
  ...
  test("strictly validates method params, results, and event payloads", () => {
    ...
    expect(parseRpcRequest({ protocolVersion: 1, type: "request", id: "preview", method: "host.getFilePreviewSource", params: { handle: "file_0123456789abcdef" } })).toEqual(expect.objectContaining({ params: { handle: "file_0123456789abcdef" } }));
    const previewBlob = new Blob(["media"], { type: "video/mp4" });
    expect(parseServiceResult("host.getFilePreviewSource", { kind: "blob", blob: previewBlob })).toEqual({ kind: "blob", blob: previewBlob });
    expect(parseServiceResult("host.getFilePreviewSource", { kind: "url", url: "https://objects.example.test/media", expiresAt: 123 })).toEqual({ kind: "url", url: "https://objects.example.test/media", expiresAt: 123 });
    expect(() => parseServiceResult("host.getFilePreviewSource", { kind: "url", url: "javascript:alert(1)", expiresAt: 123 })).toThrow("URL");
