...
export function parseInstalledApp(value: unknown): InstalledApp {
  ...
  if (Object.keys(item).some((key) => !["appId", "source", "packageEntryId", "archivePath", "sourceCatalogId", "sourceDesktopId", "sourceContentRevision", "digest", "version", "manifest", "approvedAt"].includes(key))) throw new TypeError("Installed app has an unsupported shape.");
