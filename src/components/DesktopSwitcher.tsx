  ...
  return <aside className="desktop-switcher__rail" aria-label="Desktops" onKeyDown={(event) => {
    ...
    onDismiss();
      ...
      <div className="desktop-switcher__list">
        {desktops.map((desktop) => <button className="desktop-switcher__row desktop-switcher__row--switch" type="button" key={desktop.id} data-active={desktop.id === activeDesktopId || undefined} data-desktop-switch-target aria-current={desktop.id === activeDesktopId ? "true" : undefined} onClick={() => onSwitch(desktop.id)}>
