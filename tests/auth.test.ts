...
describe("session bootstrap", () => {
  test("validates stable storage identity and display metadata", () => {
    ...
    expect(parseAuthSession({ ...authority, storageId: "opaque-account-1", directBlobOrigin, user: { displayName: "Ada" }, capabilities: { ...supported, thumbnails: "thumbnail-v1" } }).capabilities.thumbnails).toBe("thumbnail-v1");
    expect(() => parseAuthSession({ ...authority, storageId: "opaque-account-1", directBlobOrigin, user: { displayName: "Ada" }, capabilities: { ...supported, thumbnails: "legacy" } })).toThrow("thumbnail capability");
