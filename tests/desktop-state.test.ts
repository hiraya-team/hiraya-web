...
describe("desktop state", () => {
  test("accepts only complete current state", () => {
    ...
    const state = { entries: snapshot.entries, snapToGrid: snapshot.layout.snapToGrid, gridSize: snapshot.layout.gridSize, wallpaper: snapshot.layout.wallpaper, editorSettings: snapshot.editorSettings, appearance: snapshot.appearance, sync: snapshot.sync };
  ...
  test("restores a valid packaged wallpaper selection", () => {
    ...
    const state = { entries: [], snapToGrid: false, gridSize: snapshot.layout.gridSize, wallpaper: { ...DEFAULT_WALLPAPER, source: "theme:aurora" as const }, editorSettings: snapshot.editorSettings, appearance: { selectedThemeId: theme.id, customThemes: [theme] }, sync: snapshot.sync };
    expect(parseDesktopState(state)).toEqual(state);
  });
});
